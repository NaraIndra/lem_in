...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/10/26 23:03:11 by jstokes           #+#    #+#             */
/*   Updated: 2019/10/26 23:03:12 by jstokes          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
typedef struct				s_r_node
{
	int						x;
	int						y;
	void					*content;
	size_t					content_size;
	struct s_r_node			*next;
	struct s_r_node			*prev;
}							t_r_node;
...
typedef struct				s_cont
{
	...
	char					*parent;
	void					*kid;
	t_list					*deleted_layers;
	t_list					*deleted_orient;
	t_list					*deleted_bfs_shortest;
	t_list					*deleted_bfs_augm;
	t_list					*deleted_bfs_augm_index;
}							t_cont;
...
char						*ft_memjoin(char *s1,
		char *s2, size_t t1, size_t t2);
...
**RING_FUNC
*/
t_ring						*ring_create(void);
t_ring						*rnode_add_first(t_ring *ring, void *content);
t_ring						*rnode_add_last(t_ring *ring, void *content);
void						*rnode_del_target(t_ring *ring, void *target);
void						*rnode_del_last(t_ring *ring);
void						*rnode_del_first(t_ring *ring);
void						*r_del_node(t_r_node **node);
void						ring_print(t_ring *ring);
...
t_keystr_avl_t				*ft_keystr_avl_insert(t_keystr_avl_t *p,
		t_pair *pair);
t_keystr_avl_t				*ft_keystr_avl_search(t_keystr_avl_t *p,
		void *key);
t_keystr_avl_t				*ft_keystr_avl_findmin(t_keystr_avl_t *p);
t_keystr_avl_t				*ft_keystr_avl_remove(t_keystr_avl_t *p,
		void *key);
void						ft_keystr_avl_del(t_keystr_avl_t **root);
/*
**HASHMAP_FUNC
*/
...
int							hm_put(t_hash_map **map,
		void *key, t_cont *content);
...
typedef void				(*t_fptr_floatoperation)(t_buf_struct *,
		t_dec_frac *);
typedef void				(*t_fptr_chose_content)(t_arg_opt *opt,
...
**parse_and_manage
*/
int							form_str_from_arg(t_buf_struct *buf_s,
		U_CHAR **format);
...
void						manage_unsigned_num(t_buf_struct *buf_s,
		U_CHAR base);
...
void						adjust_int_width(t_buf_struct *buf_s,
		int *substract);
/*
**long_double_manager
*/
...
int							check_for_inf_nan(t_buf_struct *buf_s,
		t_float_part *p_n);
void						count_exp_pow(t_buf_struct *buf_s,
		t_dec_frac *l_num);
...
void						adjust_fnum_prec(t_buf_struct *buf_s,
		t_dec_frac *l_num);
void						adjust_double_norm_form(t_dec_frac *l_num,
		int prec, int count_prec, int *ld_zeros);
void						adjust_double_sci_form(t_buf_struct *buf_s,
		t_dec_frac *l_num);
void						adjust_sci_prec(t_buf_struct *buf_s,
		t_dec_frac *l_num, int count_prec, int cut_point);
void						analize_frac_cell(t_dec_frac *l_num, int dif_prec,
		int *ld_zeros, U_CHAR is_zero_cell);
void						analize_frac_cell_zeros(t_dec_frac *l_num,
		int index, int dif_prec, int *ld_zeros);
void						analize_frac_cell_z_p_0(t_dec_frac *l_num,
		int index, int *ld_zeros);
void						analize_frac_cell_dec(t_dec_frac *l_num,
		int dif_prec);
...
void						count_ld_num(t_buf_struct *buf_s,
		t_float_part *p_n, t_dec_frac *l_num);
int							make_ln_str(t_float_part *p_n,
		t_buf_struct *buf_s);
int							make_dec_buf(t_buf_struct *buf_s,
		t_dec_frac *l_num);
void						make_frac_buf(t_buf_struct *buf_s,
		t_dec_frac *l_num);
...
int							count_zero_i(t_buf_struct *buf_s,
		t_dec_frac *l_num, int count_prec, int index);
...
void						put_ldouble_num(t_buf_struct *buf_s,
		t_dec_frac *l_num);
void						put_ldouble_exp(t_buf_struct *buf_s,
		t_dec_frac *l_num);
...
**struct_init_func
*/
void						initfptr_operations(t_fptr_floatoperation
		*operations);
